...
class StaticDetector:
    ...
    def __init__(self, config: StaticDetectorConfig | None = None):
        self.cfg = config or StaticDetectorConfig()
    ...
    def detect(self, frames: Sequence[np.ndarray]) -> Detection:
        ...
        if any(f.shape[:2] != shape for f in frames):
            raise ValueError("Sampled frames must have identical dimensions")
        gray = np.stack(
            [cv2.cvtColor(f, cv2.COLOR_BGR2GRAY).astype(np.float32) for f in frames]
        )
        mean, var = gray.mean(0), gray.var(0)
        nv = var / (mean + 12.0)
        cutoff = np.percentile(
            nv, max(1.0, (1 - self.cfg.temporal_stability) * 100)
        )
        stable = nv <= cutoff
        edges = np.stack(
            [
                cv2.Canny(g.astype(np.uint8), 50, 150).astype(np.float32)
                for g in gray
            ]
        )
        ec = edges.mean(0) >= np.percentile(
            edges.mean(0), self.cfg.edge_percentile
        )
        candidate = (stable & ec).astype(np.uint8) * 255
        h, w = shape
        bf = max(1, int(min(h, w) * self.cfg.border_fraction))
        border = np.zeros((h, w), np.uint8)
        border[:bf, :] = border[-bf:, :] = 1
        border[:, :bf] = border[:, -bf:] = 1
        candidate = candidate & ((border | stable.astype(np.uint8)) * 255)
        k = max(3, self.cfg.morph_kernel | 1)
        ker = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (k, k))
        candidate = cv2.morphologyEx(candidate, cv2.MORPH_CLOSE, ker)
        candidate = cv2.morphologyEx(candidate, cv2.MORPH_OPEN, ker)
        n, labels, stats, _ = cv2.connectedComponentsWithStats(candidate)
        mask = np.zeros((h, w), np.uint8)
        total = float(h * w)
        for i in range(1, n):
            ratio = float(stats[i, cv2.CC_STAT_AREA]) / total
            if self.cfg.min_area_ratio <= ratio <= self.cfg.max_area_ratio:
                mask[labels == i] = 255
        if self.cfg.dilate_px > 0:
            d = 2 * self.cfg.dilate_px + 1
            mask = cv2.dilate(
                mask, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (d, d))
            )
...
class TemplateDetector:
    def __init__(self, template_path: str, threshold: float = 0.78):
        ...
        if self.template is None:
            raise ValueError(f"Cannot decode template: {p}")
    ...
    def detect(self, frame: np.ndarray) -> Detection:
        ...
        if th > gray.shape[0] or tw > gray.shape[1]:
            raise ValueError("Template is larger than input frame")
        score = cv2.matchTemplate(gray, self.template, cv2.TM_CCOEFF_NORMED)
        _, value, _, loc = cv2.minMaxLoc(score)
        mask = np.zeros(gray.shape, np.uint8)
        if value >= self.threshold:
            x, y = loc
            mask[y : y + th, x : x + tw] = 255
            return Detection(mask, float(value), (x, y, tw, th))
...
class DynamicDetector:
    ...
    def detect(self, frame: np.ndarray) -> list[Detection]:
        if self.backend is None:
            raise RuntimeError(
                "Configure a SAM-2, YOLO-World or Florence-2 backend"
            )
        result = self.backend.predict(frame)
        result = result if isinstance(result, list) else list(result)
        return [
            d
            for d in result
            if not self.label_allowlist or d.label.lower() in self.label_allowlist
        ]
...
def load_sampled_frames(video_path: str, sample_frames: int = 48) -> list[np.ndarray]:
    ...
    step = max(1, count // max(1, sample_frames))
    out = []
    try:
