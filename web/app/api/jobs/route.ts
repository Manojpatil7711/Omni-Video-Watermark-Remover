...
export async function POST(request: Request) {
  ...
  if (!workerUrl || !workerKey) return NextResponse.json({ error: "Worker is not configured" }, { status: 503 });

  const body = await request.json().catch(() => null);
  if (!body || typeof body.input_url !== "string" || !body.input_url) {
  ...
  const response = await fetch(`${workerUrl.replace(/\/$/, "")}/v1/jobs`, {
    ...
    body: JSON.stringify({ input_url: body.input_url, mode: body.mode ?? "auto", backend: body.backend ?? "auto" }),
    cache: "no-store",
  });
